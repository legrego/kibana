...
export default function ({ getService }: FtrProviderContext) {
  ...
  describe('POST /api/saved_objects/_export', () => {
    before(async () => {
      await esArchiver.load('confidential_objects');
    });

    after(async () => {
      await esArchiver.unload('confidential_objects');
    });
    ...
    > = {
      httpCode: 200,
      expectResponse: ({ expectedResults, missingRefCount = 0, missingReferences = [] }) => (
        response
      ) => {
