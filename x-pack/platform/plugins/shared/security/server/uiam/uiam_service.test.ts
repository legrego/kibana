...
describe('UiamService', () => {
  ...
  describe('#createServiceAccount', () => {
    ...
    // A client-asserted SAN header would be forgeable; we rely on transport-level mTLS instead.
    it('does not send an `X-Client-Sans` header', async () => {
      fetchSpy.mockResolvedValue({ ok: true, json: async () => ({ id: 'service-account-id' }) });

      await uiamService.createServiceAccount('access-token', body);

      const [, requestInit] = fetchSpy.mock.calls[0];
      expect(Object.keys(requestInit.headers).map((header) => header.toLowerCase())).not.toContain(
        'x-client-sans'
      );
    });
