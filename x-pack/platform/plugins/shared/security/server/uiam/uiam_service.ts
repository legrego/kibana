...
export class UiamService implements UiamServicePublic {
  ...
  /**
   * See {@link UiamServicePublic.createServiceAccount}.
   */
    ...
    try {
      ...
      const response = await UiamService.#parseUiamResponse(
        await fetch(`${this.#config.url}/uiam/api/v1/service-accounts`, {
          ...
          body: JSON.stringify(body),
          // The API specification also documents an `X-Client-Sans` header carrying the mTLS SANs
          // of the calling service. We deliberately do not send it: a client-asserted SAN header
          // would be forgeable, defeating the point of mTLS. mTLS itself is applied at the
          // transport layer by `#createFetchDispatcher`, and we expect UIAM's TLS-terminating
          // proxy to derive the header from the certificate we already present.
          // @ts-expect-error Undici `fetch` supports `dispatcher` option, see https://github.com/nodejs/undici/pull/1411.
          dispatcher: this.#dispatcher,
        })
      );
